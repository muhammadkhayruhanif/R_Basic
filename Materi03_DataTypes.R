...
m1 <- matrix(c(T, T, F, F, T, F), nrow = 2) # Jumlah baris=2
m1

m2 <- matrix(c("a", "b", 
               "c", "d"), 
               nrow = 2,
               byrow = T) # Diurutkan berdasarkan baris
m2
...
a1 <- array(c( 1:24), c(4, 3, 2)) # Diurutkan sampai angka akhir
a1
...
## Coba Matrix
umur <- c(19, 24, 27, 18, 21)
nama <- c("Hanafi", "Huza", "Banu", "Fikra", "Fazli")
kuliah <- c(TRUE, TRUE, FALSE, F, T)

dataBind <- cbind(nama, umur, kuliah)
dataBind

dataFrame <- data.frame(nama, umur, kuliah)
dataFrame
## List ====================================================
